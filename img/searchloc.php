...
<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDGxzkDyvxPpkRRm8DWxRjNpwPJONzHkDA"></script>
<script>
var geocoder;
var map;
var marker;
...
function initialize() {
    ...
    initialLat = initialLat?initialLat:8.9800689;
    initialLong = initialLong?initialLong:38.7989319;
        ...
        geocoder.geocode({'latLng': marker.getPosition()}, function (results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    ...
                    $('.search_latitude').val(marker.getPosition().lat());
                    $('.search_longitude').val(marker.getPosition().lng());
                   //	 $('.search_lat').val(marker.getPosition().lng());
